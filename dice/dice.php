...
class Dice {
	...
	public function addRule($name,array $rule) {
		if (isset($rule['instanceOf']) &&
			(!array_key_exists('inherit',$rule) || $rule['inherit']===TRUE)) $rule=
			array_replace_recursive($this->getRule($rule['instanceOf']),$rule);
		$this->rules[ltrim(strtolower($name),'\\')]=
			array_replace_recursive($this->getRule($name),$rule);
	}
	...
	public function getRule($name) {
		...
		{                            // Find a rule which matches the class described in $name where:
			if (empty($rule['instanceOf'])
				...
				&& (!array_key_exists('inherit',$rule) || $rule['inherit']===
					TRUE)) // And that rule should be inherited to subclasses
				return $rule;
		}
...
//		if (empty($this->cache[$name]))
	...
	private function getClosure($name,array $rule) {
		...
		else if (!empty($rule['shared'])) $closure=
			function(array $args,array $share) use ($class,$name,$constructor,$params) {
				// Shared instance: create the class without calling the constructor (and write to \$name and $name, see issue #68)
				$this->instances[$name]=$this->instances[ltrim($name,'\\')]=
				...
				if ($constructor) $constructor->invokeArgs($this->instances[$name],
					$params($args,$share));
				return $this->instances[$name];
			};
		else if ($params) $closure=
			function(array $args,array $share) use ($class,$params) {
		...
		if (isset($rule['shareInstances'])) $closure=
			function(array $args,array $share) use ($closure,$rule) {
		...
		return isset($rule['call'])?
			function(array $args,array $share) use ($closure,$class,$rule) {
				...
				foreach ($rule['call'] as $call) {
					...
					if (isset($call[2]) && is_callable($call[2])) call_user_func($call[2],
						$return);
				}
				return $object;
			}:$closure;
	...
	private function expand($param,array $share=[],$createFromString=FALSE) {
		if (is_array($param) && isset($param['instance'])) {
			...
			if (is_array($param['instance'])) $param['instance'][0]=
				$this->expand($param['instance'][0],$share,TRUE);
			if (is_callable($param['instance'])) return call_user_func($param['instance'],
				...$args);
			else return $this->create($param['instance'],array_merge($args,$share));
		} // Recursively search for 'instance' keys in $param
		else if (is_array($param)) foreach ($param as $name=>$value)
	...
	private function getParams(\ReflectionMethod $method,array $rule) {
		...
		return function(array $args,array $share=[]) use ($paramInfo,$rule) {
			...
			if ($share || isset($rule['constructParams'])) $args=array_merge($args,
				isset($rule['constructParams'])?
			...
			foreach ($paramInfo as list($class,$param,$sub)) {
				...
				if ($class) try {
					$parameters[]=
						$sub?$this->expand($rule['substitutions'][$class],$share,TRUE):
